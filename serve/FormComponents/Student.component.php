<?php
  /*
   * for a future reader/developer:
   * the use of this file was to build the frontface
   * as fast as possible, this meant a lot of copy-pasting,
   * etc... in the future I'd love to write a module
   * that takes in an array and generates all of this information
   * out, a sort of "builder" that works for that.
   * that means templates are able to be used, and other functionalities
   * can quickly be developed.
   *
   * This is the file which I want to make a hotspot for quick
   * future-editing by enabling new precedures to help with the
   * amount of reusability that is not present in the current file.
   * PHP IS for HTML parsing so we're going to take it back to that.
   *
   * building indipendant functions which can be called from themselves
  ...
  <div data-id="<?=$id?>" data-type="students">
    <div class="box row formation">
      ...
      <h4 class="students-title gold" style="width: 100%;">
        ...
        <?php if (!$first): ?>
          <!--BIC ERROR, THE DELETE BIT IS DELETE-PARENT-->
